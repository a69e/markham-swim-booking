...
export async function saveAttendees(db, accountId, html) {
  ...
  const savedDefaultRows = await db`
    select account_attendees.id, account_attendees.full_name
    from account_credentials
    join account_attendees
      on account_attendees.id = account_credentials.default_attendee_id
      and account_attendees.account_id = account_credentials.id
    where account_credentials.id = ${accountId}
    limit 1
  `;
  ...
  for (const attendee of attendees) {
    await db`
      ...
      do update set
        ...
        is_default = account_attendees.is_default,
        updated_at = now()
    `;
  }
  ...
  const existingDefaultRows = await db`
    ...
    limit 1
  `;
  const defaultRow = savedDefaultRows[0] || existingDefaultRows[0] || ownerRows[0] || null;
