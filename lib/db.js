...
export async function ensureQueueSchema() {
  ...
  await db`
    alter table queued_sessions
    drop constraint if exists queued_sessions_device_id_session_key_key
  `;
...
export async function accountScopeForDevice(db, deviceId) {
  ...
  if (!account) {
    return { account: null, accountIds: [], email: "" };
  }
  ...
  return {
    account,
    accountIds: scopedRows.map((row) => row.id),
    email: account.email,
  };
}
