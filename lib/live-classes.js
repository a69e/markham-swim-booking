...
export function normalizeClass(item) {
  return {
    ...
    locationId: item.Address?.Id || item.LocationId || "",
...
export async function fetchLiveClassBatch(after = "") {
...
export async function fetchLiveClassPages(limit = 5) {
  const sessions = [];
  let nextKey = "";
  let hasMore = true;

  for (let index = 0; index < limit && hasMore; index += 1) {
    const batch = await fetchLiveClassBatch(nextKey);
